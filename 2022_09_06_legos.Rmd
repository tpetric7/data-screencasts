...
lego_datasets11 <- lego_datasets[11] %>% 
  tibble() %>% 
  unnest(cols = c(.))
...
lego_datasets12 <- lego_datasets[12] %>% tibble() %>% unnest(c(.))

sets_with_themes <- lego_datasets11 %>% 
# sets_with_themes <- lego_datasets$sets %>%
  left_join(lego_datasets12 %>%
...
```{r}
lego_datasets03 <- lego_datasets[3] %>% tibble() %>% unnest(c(.))
lego_datasets05 <- lego_datasets[5] %>% tibble() %>% unnest(c(.))
lego_datasets01 <- lego_datasets[1] %>% tibble() %>% unnest(c(.))

# Most recent version of each inventory for each set_num
inventories_current <- lego_datasets03 %>%
...
set_parts <- sets_with_themes %>%
  ...
  inner_join(lego_datasets05, by = "inventory_id", suffix = c("", "_inventory")) %>%
  # inner_join(lego_datasets$inventory_parts, by = "inventory_id", suffix = c("", "_inventory")) %>%
  left_join(lego_datasets01 %>%
