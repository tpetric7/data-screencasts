...
```{r}
exdir = getwd()
unzip(paste0(exdir, "/sliced-s01e11-semifinals.zip"), exdir = exdir)
```
...
```{r}
xg_fit <- xg_wf %>%
  finalize_workflow(select_best(xg_tune2)) %>%
  fit(train)
...
parsnip::set_dependency("boost_tree", eng = "catboost", "catboost")
parsnip::set_dependency("boost_tree", eng = "catboost", "treesnip")
